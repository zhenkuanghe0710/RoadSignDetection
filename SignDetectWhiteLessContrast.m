function outputW = SignDetectWhiteLessContrast(I, outputR)
% Generatie invert C.
IC = ones(3);
MeanVector = [1 1 1];
...
for i = 1:lmax
    ...
    if length(r) > 100 && length(r) <3000
        ...
        if f_v > 50 && l_v < l-50 && f_h > 50 && l_h < w-50
            if long > wide && wide / long > 0.5 && wide / long < 0.85
                if (count0c / count1c) <0.25 && (count0 /count1) > 0.3
                    if long * wide > 600
...
% If no match result, detect the dark place           
else
outputW = SignDetectWhiteDark(I, outputR);    
end
return